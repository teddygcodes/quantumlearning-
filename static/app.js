...
import { generateRandomProblem, checkAnswer } from './problems.js';
...
function showChapterDetail(ch, cs) {
  ...
  modal.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:16px;">
      ...
      <button class="btn btn-ghost" style="padding:8px 12px;font-size:18px;" onclick="window.__closeModal2()">✕</button>
    </div>
    ...
    <div style="display:flex;flex-direction:column;gap:12px;">
      <button class="btn btn-blue btn-full" onclick="navigate('/lesson/${ch.id}');closeModal();">
        📖 Start Lesson
      </button>
      <button class="btn btn-green btn-full" onclick="navigate('/practice/${ch.id}');closeModal();">
        ✏️ Practice Problems
      </button>
      <button class="btn ${cs.unlocked || cs.completed ? 'btn-ghost' : 'btn-ghost'} btn-full"
              ${!cs.unlocked && !cs.completed ? 'disabled style="opacity:0.4;cursor:default;"' : ''}
              onclick="navigate('/quiz/${ch.id}');closeModal();">
        🎯 Take Quiz (${ch.quizPass}/${ch.quizCount} to pass)
      </button>
    </div>
  `;

  window.__closeModal2 = closeModal;
...
function renderLesson(chapterId) {
  ...
  setContent(`
    <div class="lesson-view" data-ch="${chapterId}" style="--ch-color:var(--ch${chapterId});">
      <div class="problem-topbar">
        <button class="btn btn-ghost" style="padding:8px 16px;" onclick="navigate('/');">← Back</button>
        <div style="flex:1;"></div>
      </div>
      <div style="padding:20px 20px 120px;">
        ${ch.lessonHTML}
      </div>
      <div style="position:fixed;bottom:0;left:0;right:0;padding:16px 20px;background:linear-gradient(transparent, var(--bg) 40%);">
        <button class="btn btn-green btn-full" onclick="navigate('/practice/${chapterId}');">
    ...
    <div class="problem-screen" data-ch="${chapterId}" style="--ch-color:var(--ch${chapterId});--ch-dk:var(--ch${chapterId}-dk);">
      ...
      <div class="problem-left">
        ...
        <div class="answer-section">
          <label style="font-size:13px;color:var(--text-muted);font-weight:800;text-transform:uppercase;letter-spacing:0.5px;display:block;margin-bottom:8px;">
            Your Answer
          </label>
        ...
        <div id="result-actions" style="display:none;flex-direction:column;gap:10px;margin-top:4px;">
          ${!isQuiz ? '<button class="btn btn-ghost btn-full" id="btn-similar" onclick="window.__trySimilar();">Try Similar</button>' : ''}
          <button class="btn btn-blue btn-full" id="btn-next" onclick="window.__next();">
            ${isQuiz ? 'Next Question →' : 'Next Problem →'}
          </button>
        </div>
